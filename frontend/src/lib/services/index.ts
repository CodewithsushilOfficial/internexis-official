...
api.interceptors.request.use(
  (config) => {
    ...
    if (
      adminToken &&
      (config.url?.includes("/admin/") || config.url?.includes("/api/admin/"))
    ) {
...
export const campusAmbassadorService = {
  async submitApplication(data: CampusAmbassadorFormData) {
    ...
    if (!data.motivation?.trim())
      errors.push("Why you want to be a Campus Ambassador is required");
...
export const adminService = {
  ...
  async getApplicationsByType(
    type: "ambassador" | "career" | "internship",
    params?: {
      page?: number;
      limit?: number;
      status?: string;
    ...
    try {
      ...
      throw {
        ...
        message:
          axiosError.response?.data?.message ||
          `Failed to fetch ${type} applications`,
      };
    }
  },
  ...
  async updateApplicationStatus(
    ...
    try {
      const response = await api.patch(
        `/api/admin/applications/${type}/${id}/status`,
        { status },
      );
      return response.data;
    } catch (error: unknown) {
  ...
  async deleteApplication(
    ...
    try {
      console.log(
        `🗑️ Admin Service: Deleting ${type} application with ID:`,
        id,
      );

      const response = await api.delete(
        `/api/admin/applications/${type}/${id}`,
      );

      console.log(
        "✅ Admin Service: Application deleted successfully:",
        response.data,
      );
      return response.data;
    } catch (error: unknown) {
  ...
  async exportApplications(
    ...
    try {
      console.log(
        `📤 Admin Service: Exporting ${type} applications in ${format} format`,
      );
      ...
      return {
        success: true,
        message: `${type} applications exported successfully`,
      };
    } catch (error: unknown) {
      ...
      return {
        ...
        message:
          axiosError.response?.data?.message ||
          `Failed to export ${type} applications`,
      };
    }
  },
  ...
  async exportAllApplications() {
    try {
      ...
      return {
        success: true,
        message: "All applications exported successfully",
      };
    } catch (error: unknown) {
      ...
      return {
        ...
        message:
          axiosError.response?.data?.message ||
          "Failed to export all applications",
      };
    }
  },
};
