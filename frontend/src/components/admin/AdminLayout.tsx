...
const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <div
      className={`flex flex-col h-full ${mobile ? "bg-white" : "bg-gray-900"}`}
    >
      {/* Logo */}
      <div
        className={`flex items-center justify-between h-16 px-4 ${mobile ? "border-b border-gray-200" : ""}`}
      >
        <div className="flex items-center">
          <div
            className={`w-8 h-8 rounded-lg ${mobile ? "bg-indigo-600" : "bg-indigo-500"} flex items-center justify-center`}
          >
            <span className="text-white font-bold text-sm">I</span>
          </div>
          <span
            className={`ml-2 text-lg font-semibold ${mobile ? "text-gray-900" : "text-white"}`}
          >
            Internexis Admin
          </span>
        </div>
      ...
      {/* Bottom section */}
      <div
        className={`px-2 py-4 border-t ${mobile ? "border-gray-200" : "border-gray-700"}`}
      >
        <button
          onClick={onLogout}
          className={`w-full group flex items-center px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${
