...
import { Search, Loader2, Clock, AlertCircle, CheckCircle, FileSearch, Mail } from 'lucide-react';
import { useTheme } from '../../../lib/hooks/use-theme';

// Create a sample status type for demonstration
type ApplicationStatus = 'pending' | 'reviewing' | 'approved' | 'rejected' | null;
  ...
  const getStatusDisplay = () => {
    ...
    const statusConfig = {
      pending: {
        ...
        color: "bg-amber-100 dark:bg-amber-900/30 border-amber-200 dark:border-amber-700",
        step: 1
      },
      reviewing: {
      ...
      rejected: {
        ...
        color: "bg-red-100 dark:bg-red-900/30 border-red-200 dark:border-red-700",
        step: 3
      }
    };
    ...
    const steps = [
      ...
      { name: 'Decision', icon: status === 'approved' ? <CheckCircle /> : <AlertCircle />, step: 3 }
    ];

    return (
      <div className={`mt-8 p-6 rounded-lg border ${currentStatus.color}`} style={{animation: "fadeIn 0.5s ease-out forwards"}}>
        <div className="flex flex-col md:flex-row items-center gap-4">
          <div className="p-3 rounded-full bg-white dark:bg-gray-800 shadow-md">
            {currentStatus.icon}
          </div>
          <div className="text-center md:text-left">
            <h3 className="font-bold text-xl dark:text-gray-100">{currentStatus.title}</h3>
            <p className="text-gray-700 dark:text-gray-300">{currentStatus.desc}</p>
          </div>
        </div>
                  ...
                  {index > 0 && (
                    <div 
                      className={`absolute h-1 ${
                        isActive ? (isRejected ? 'bg-red-400' : 'bg-blue-500') : 'bg-gray-300 dark:bg-gray-600'
                      }`}
                      style={{ 
        ...
        <div className="mt-6 space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium dark:text-gray-200">{new Date().toLocaleDateString()}</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 transition-colors duration-200">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-8 text-center">
              Stay updated on your application progress with real-time status tracking
            </p>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <ul className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-start">
                  ...
                  <span>Reference ID can be found in your confirmation email</span>
                </li>
                <li className="flex items-start">
                  <div className="min-w-[20px] mr-2">•</div>
                  <span>Status updates are typically processed within 24-48 hours</span>
                </li>
                <li className="flex items-start">
                  <div className="min-w-[20px] mr-2">•</div>
                  <span>For immediate assistance, please use the chat support</span>
                </li>
                <li className="flex items-start">
                  <div className="min-w-[20px] mr-2">•</div>
                  <span>Type "approved", "rejected", or "review" in your search to see different statuses for demo purposes</span>
                </li>
              </ul>
            </div>
          </div>
