...
const getYouTubeVideoId = (url: string): string => {
  ...
  else if (url.includes("youtube.com/watch")) {    try {
...
const SecureYouTubeEmbed: React.FC<SecureYouTubeEmbedProps> = ({ videoUrl, title, resources }) => {
  ...
  const getGitHubUrl = () => {
    ...
    const sourceCodeResource = resources.find(resource => 
  ...
  if (!videoId) {
    return (
      <div className="aspect-video w-full rounded-t-2xl overflow-hidden relative bg-gray-200 flex items-center justify-center flex-col gap-4">
        <p className="text-gray-700 font-medium">Video tutorial not available</p>
        {githubUrl && (
          <a 
            href={githubUrl} 
            target="_blank" 
