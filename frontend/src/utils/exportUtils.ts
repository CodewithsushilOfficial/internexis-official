// Security Note: xlsx package has known vulnerabilities (GHSA-4r6h-8v6p-xvw6, GHSA-5pgg-2g8v-p4x9)
// This utility is only used in admin functionality and should be used with caution
...
export class ExportManager {
  ...
  private static getApplicationTypeHeaders(
    type: string,
  ...
  static exportAllToExcel(
    ...
    const ambassadorSheet = createStyledWorksheet(
      ambassadors,
      "campus-ambassadors",
      "4472C4",
    );
    const careerSheet = createStyledWorksheet(careers, "careers", "10B981");
    const internshipSheet = createStyledWorksheet(
      internships,
      "internships",
      "8B5CF6",
    );
  ...
  static exportSummaryReport(
    ...
    if (applications.ambassadors.length > 0) {
      const ambassadorSheet = this.createStyledWorksheet(
    ...
    if (applications.careers.length > 0) {
      const careerSheet = this.createStyledWorksheet(
    ...
    if (applications.internships.length > 0) {
      const internshipSheet = this.createStyledWorksheet(
  ...
  private static createStyledWorksheet(
    data: ApplicationData[],
    type: string,
    color: string,
  ) {
